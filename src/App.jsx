...
import {
  ...
  Lightbulb, AlertTriangle, Play, Home, Code
} from 'lucide-react';
import './index.css';
...
function App() {
  ...
  const saveProgress = (lessonId) => {
    ...
    localStorage.setItem('javascript-progress', JSON.stringify(updated));
  };
                ...
                {rows.map((row, ri) => (
                  <tr key={ri}>{row.map((cell, ci) => (
                    <td key={ci} style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', color: 'var(--text-secondary)' }}>
                      <span dangerouslySetInnerHTML={{ __html: cell.replace(/\*\*(.*?)\*\*/g, '<strong style="color:var(--text-primary)">$1</strong>').replace(/`(.*?)`/g, '<code style="background:rgba(245,158,11,0.1);padding:2px 6px;border-radius:4px;font-size:13px">$1</code>') }} />
                    </td>
                  ))}</tr>
                ))}
              </tbody>
      ...
      if (line.startsWith('- ') || line.startsWith('* ')) {
        ...
        elements.push(
          <ul key={`ul-${i}`}>{items.map((item, idx) => (
            <li key={idx} dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code style="background:rgba(245,158,11,0.1);padding:2px 6px;border-radius:4px">$1</code>') }} />
          ))}</ul>
        );
        continue;
      }
      ...
      if (line.trim()) {
        elements.push(
          <p key={`p-${i}`} dangerouslySetInnerHTML={{ __html: line
            ...
            .replace(/`(.*?)`/g, '<code style="background:rgba(245,158,11,0.1);padding:2px 6px;border-radius:4px;font-family:JetBrains Mono,monospace;font-size:14px">$1</code>')
            .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" style="color:var(--accent-secondary)">$1</a>')
          }} />
        );
      }
      i++;
  ...
  return (
    <div className="app-container">
      ...
      <div className="gradient-orb orb-tertiary" />
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="sidebar-header">
          <div className="sidebar-logo"><Code size={24} /></div>
      ...
      <main className="main-content">
        {currentView === 'home' && (
          <>
            <div className="hero">
              <div className="hero-badge"><Code size={16} /><span>JavaScript Mastery Course</span></div>
              <h1 className="hero-title">Master<br /><span>JavaScript</span></h1>
              <p className="hero-subtitle">From fundamentals to advanced patterns. Learn modern JavaScript with hands-on examples and real-world projects.</p>
            ...
            <div className="glass-card" style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Master modern JavaScript from basics to advanced concepts:</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px' }}>
                {['Variables & Types', 'Functions', 'DOM Manipulation', 'Async/Await', 'ES6+ Features', 'Best Practices'].map((item) => (
                  <div key={item} style={{ padding: '16px', background: 'rgba(245, 158, 11, 0.1)', borderRadius: '12px', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
